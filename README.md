...
| `DOWNLOAD_BUDGET`            | Time budget for finding a server that has the blob, bounding worst-case miss cost (excludes the transfer itself)            | `30000` (30s)                                       |
...
| `WORKER_RESPAWN_DELAY`       | Delay before replacing a lost download worker (rate-limits crash loops)                                                     | `1000` (1s)                                         |
| `REQUEST_IDLE_TIMEOUT`       | Seconds a client connection may sit without traffic before Bun closes it (max `255`)                                        | `120` (2m)                                          |
